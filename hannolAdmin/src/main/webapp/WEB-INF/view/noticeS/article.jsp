...
<c:if test="${sessionScope.staff.authority=='ROLE_ADMIN' || sessionScope.staff.staffIdx==dto.usersCode}">
	...
	var url = "<%=cp%>/noticeS/delete?" + query;
...
<c:if test="${sessionScope.staff.staffIdx==dto.usersCode}">
	...
	var page = "${page}";
	var query = "num="+num+"&${query}";
	var url = "<%=cp%>/noticeS/update?" + query;
			    ...
			    <td colspan="2" align="left" style="padding-left: 5px;">
			       이전글 :
			         <c:if test="${not empty preReadDto}">
			              <a href="<%=cp%>/noticeS/article?${query}&num=${preReadDto.noticeCode}">${preReadDto.subject}</a>
			        </c:if>
			    </td>
			</tr>
			    ...
			    <td colspan="2" align="left" style="padding-left: 5px;">
			       다음글 :
			         <c:if test="${not empty nextReadDto}">
			              <a href="<%=cp%>/noticeS/article?${query}&num=${nextReadDto.noticeCode}">${nextReadDto.subject}</a>
			        </c:if>
			    </td>
			</tr>
			</table>
			...
			<tr height="45">
			    <td width="300" align="left">
			       ...
			       <c:if test="${sessionScope.staff.staffIdx==dto.usersCode || sessionScope.staff.authority=='ROLE_ADMIN'}">				    
			          <button type="button" class="btn btn-default" onclick="deleteNotice();">삭제</button>
			       </c:if>
			    </td>
			
			    <td align="right">
			        <button type="button" class="btn btn-default" onclick="javascript:location.href='<%=cp%>/noticeS/list?${query}';">리스트</button>
			    </td>
			</tr>
			</table>
    </div>
